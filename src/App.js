...
class App extends Component {
  ...
  myPigLatinCodeHere = () => {
    ...
    let translatedWordsArray = userInput.map((currentWord) => {
      ...
      // y
      // if there is no vowels y is a vowels
      // qu
      // if q and u are together - consonant move on to the next vowels
      else if (currentWord.match(/[aeiou]/g) === null) {
        let yIndex = currentWord.indexOf("y");
        translatedStr = `${currentWord.substring(
          yIndex
        )}${currentWord.substring(0, yIndex)}ay`;
        return translatedStr;
      } else if (currentWord.match(/["qu"]/g)) {
        let noUVowl = currentWord.match(/[aeio]/g) || 0;
        let vowelIndexNoU = currentWord.indexOf(noUVowl[0]);
        translatedStr = `${currentWord.substring(
          vowelIndexNoU
        )}${currentWord.substring(0, vowelIndexNoU)}ay`;
        return translatedStr;
      } else {
        ...
        translatedStr = `${currentWord.substring(
          vowelIndex
        )}${currentWord.substring(0, vowelIndex)}ay`;
        return translatedStr;
      }
